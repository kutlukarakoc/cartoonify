...
const TabsList = React.forwardRef<TabsPrimitive.ListRef, TabsPrimitive.ListProps>(
  ({ className, ...props }, ref) => (
    <TabsPrimitive.List
      ref={ref}
      className={cn(
        'h-10 native:h-12 items-center justify-center rounded-md bg-[#d4c3b0] p-1 native:px-1.5',
        className
      )}
      {...props}
    />
      ...
      >
        <TabsPrimitive.Trigger
          ...
          className={cn(
            ...
            props.value === value && 'bg-[#8b7355] shadow-lg shadow-foreground/10',
            className
          )}
          {...props}
        />
